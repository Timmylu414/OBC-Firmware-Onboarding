...
#define LM75BD_REG_TEMP 0x00U /* Temp register*/
...
error_code_t readTempLM75BD(uint8_t devAddr, float *temp)
{
  ...
  error_code_t errorCode = i2cSendTo(devAddr, &tempBuf, 1);
  if (errorCode != ERR_CODE_SUCCESS)
  {
    return errorCode;
  }
  uint8_t data[2] = {0};
  errorCode = i2cReceiveFrom(devAddr, data, 2);
  if (errorCode != ERR_CODE_SUCCESS)
  {
    return errorCode;
  }

  uint16_t tempData = data[0] << 8;
